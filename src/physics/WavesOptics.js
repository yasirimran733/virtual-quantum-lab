 ...
 * @param {number} params.slitWidth - Slit width (m)
 * @param {number} params.distance - Distance to screen (m)
...
export const calculateReflection = (params = {}) => {
  const {
    incidentRay = { direction: { x: 1, y: 0, z: 0 }, position: { x: 0, y: 0, z: 0 } },
    normal = { x: 0, y: 1, z: 0 },
  } = params
  ...
  const reflected = incident.clone().sub(n.clone().multiplyScalar(2 * dotProduct))
